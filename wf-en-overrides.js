...
<div class="page">
<section class="hero product-hero"><div class="product-hero-stage"><div class="product-hero-visual clickable" data-go="pdp"><span class="photo-label">Featured product in use / hero image</span></div><div class="product-hero-panel"><div class="product-hero-top"><div class="eyebrow">Featured tool / TABLE</div><span class="product-hero-index">01 / 03</span></div><h1 class="display h1">Fingertip Tongs</h1><p class="lead">Precise control for small foods, plating and everyday tabletop use.</p><div class="row action-row"><a class="btn black" data-go="pdp">View Product</a><a class="btn ghost" data-go="inuse" data-target="use-small">See in use</a></div><div class="product-hero-bottom"><span>TABLE / Featured product</span><div class="pager"><span class="on"></span><span></span><span></span></div></div></div></div></section>
<section class="section compact"><div class="brand-intro"><div class="eyebrow">The AUX difference</div><h2 class="display h2">Made with Japanese precision.</h2><p class="lead">Thin tips, task-specific shapes and compact proportions give you more control in the small movements of cooking, serving and the table.</p><div class="row action-row" style="justify-content:center"><a class="btn" data-go="why">Why AUX</a></div></div></section>
<section class="section"><div class="eyebrow">Featured tools</div><div class="row section-head-row"><div><h2 class="display h2">Tools worth reaching for.</h2></div><div class="row"><span class="arrow">←</span><span class="arrow">→</span></div></div><div class="product-strip">${[0,1,2,3].map(i=>productCard(i)).join('')}</div><div class="slider-foot"><a class="btn" data-go="shop">Shop All</a><div class="pager"><span class="on"></span><span></span><span></span></div></div></section>
...
const collectionENData={
  COOK:{
    ...
    precisionLead:'Tip shape, overall form and proportion are tuned to the task so the tool fits the movement.'
  },
  SERVE:{
    ...
    precisionLead:'Contact point, overall form and proportion work together to make serving feel more exact.'
  },
  TABLE:{
...
<div class="page">
<div class="collection-hero"><div class="collection-copy"><div class="eyebrow">Collection</div><h1 class="display h1">${name}</h1><p class="lead">${d.tagline}</p><p class="tiny">${d.sub}</p></div><div class="collection-visual"><span class="photo-label">${name} / category lifestyle image</span></div></div>
<section class="section compact"><div class="eyebrow">Why ${name.toLowerCase()}</div><h2 class="display h2">${d.valueTitle}</h2><p class="lead">${d.valueLead}</p>${name==='TABLE'?'<div class="tiny collection-note">Concept to explore: The Third Utensil / personal-scale tabletop use</div>':''}<div class="collection-benefits">${d.scenes.map((s,i)=>`<div class="collection-benefit"><b>0${i+1}</b><h3 class="h3">${s[0]}</h3><p>${s[1]}</p></div>`).join('')}</div></section>
...
<section class="section soft"><div class="split"><div>${pic('metal',name+' / engineering detail')}</div><div><div class="eyebrow">Precision behind the category</div><h2 class="display h2">${d.precisionTitle}</h2><p class="lead">${d.precisionLead}</p><div class="row action-row"><a class="btn black" data-go="why">Why AUX</a><a class="btn" data-go="inuse">See in use</a></div></div></div></section>
</div>${footer()}`}
