...
function productCard(i,target='pdp'){const p=products[i%products.length];return `<article class="product-card clickable" data-go="${target}"><div class="product-img ${i%3===1?'warm':i%3===2?'cool':''}"></div><div class="product-name">${p[0]}</div><div class="product-sub">${p[2]}</div><div class="price">$—</div></article>`}
function header(){return `<div class="site-header"><div class="logo" data-go="home">AUX</div><div class="header-right"><nav class="globalnav"><button data-go="shop">SHOP</button><button data-go="cook">COOK</button><button data-go="serve">SERVE</button><button data-go="table">TABLE</button><button data-go="inuse">IN USE</button><button data-go="why">ABOUT</button></nav><div class="actions"><button class="action-item">${searchIcon}<span>Search</span></button><button class="action-item">${cartIcon}<span>Cart (0)</span></button></div></div></div>`}
