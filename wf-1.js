...
const productPhotos=['product-pair-minimal.webp','product-pair-shadow.webp','product-pair-warm.webp','product-lineup-marble.webp','product-lineup-warm.webp'];
function productCard(i,target='pdp'){const p=products[i]||products[i%products.length],photo=productPhotos[i%productPhotos.length];return `<article class="product-card clickable" data-go="${target}"><div class="product-img ${i%3===1?'warm':i%3===2?'cool':''}"><img class="wf-photo" src="assets/wireframe/${photo}" alt=""></div><div class="product-name">${p[0]}</div><div class="product-sub">${p[2]}</div><div class="price">$—</div></article>`}
